...
def get_openai_shopping_analysis(wb):
    ...
    # 3. 프롬프트 작성
    prompt = f"""다음 두 목록을 비교분석하여, prev_list 목록에서 now_list목록으로 바뀐 주요 특징을 추출해 줘.
    prev_list 목록 : {prev_data}
    now_list 목록 : {now_data}
    비교 분석 결과를 바탕으로 구체적인 수치, 상품명, 쇼핑몰명 등을 언급하여 한글로 300자 이내로 분석글을 작성해 줘. """
    return aiconn(prompt=prompt)

def get_openai_news_summarize(str_news_data):
    # 뉴스 요약 return
    prompt = f"""다음 뉴스 내용을 구체적인 수치, 고유명사를 언급하면 글머리를 활용하여 한글 500자 이내로 요약 글을 작성해 줘.
    뉴스 내용 {str_news_data}"""
    return aiconn(prompt=prompt)