...
# 2. 백업(파일명: genai_rap2507011259.xls)
    ...
    # 4. 'now_list' 시트를 복사하여 'prev_list'시트로 시트이름 변경
...
def update_now_list(wb, df_shopping):
    # 6. 'now_list'시트에 df_shopping내용('A1'셀)을 업데이트
    now_sheet = wb.sheets['now_list']
    now_sheet.clear()
...
def update_now_report(wb, analysis, summary):
    ...
    if 'now_report' in sheet_names:
        ...
        print('now_report 시트가 없어서 작업을 중단')
    ...
    now_sheet.range('A3').api.HorizontalAlignment = xw.constants.HAlign.xlHAlignRight  # 사용시 import xlwings as xw 필요
