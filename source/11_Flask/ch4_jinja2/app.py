...
# 1. .venv\Scripts\activate (command prompt) = ctrl+j, ctrl+`) : 가상환경 들어가기
# 2. pip install flask : 가상환경내 flask 설치

## Jinja2 template 문법 ##
# 1. 변수 {{var명}} 또는 {{var명|filter}} 사용
    # 기본제공 필터 : lower, upper, capitalize, length, replace, trim,
    #                int, float, string
# 2. 제어문
...
app = Flask(__name__,
            ...
            static_folder='static')      # 정적 파일 폴더 지정(css, js, img, ...)
...
def index():
    if request.method == 'GET':
        ...
        names_list.append(name.strip()) # post방식으로 넘어온 name append
        name_length = len(name) # post방식으로 넘어온 name 길이 구하기
    price = 12000
