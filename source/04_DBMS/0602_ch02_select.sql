...
SELECT EMPNO "사 번", ENAME "이 름", SAL "급 여", JOB "직 책" FROM EMP;
SELECT EMPNO "사 번", ENAME 이름, SAL "급 여", JOB 직책 FROM EMP;
SELECT EMPNO NO, ENAME NAME, SAL SALARY FROM EMP;

-- 3. 특정 행 출력 : WHERE절(조건절) - 비교연사자 : 같다(=), 다르다(!=, ^=, <>), >, >=, <, <=
SELECT EMPNO NO, ENAME, SAL FROM EMP WHERE SAL=3000;
SELECT EMPNO NO, ENAME, SAL FROM EMP WHERE SAL^=3000;
SELECT EMPNO NO, ENAME, SAL FROM EMP WHERE SAL<>3000;
SELECT EMPNO NO, ENAME, SAL FROM EMP WHERE SAL!=3000;
    -- 비교연산자는 숫자, 문자, 날짜형 모두 가능
    -- ex1) 사원이름(ENMAE)이 'A', 'B', 'C'로 시작하는 사원의 모든 필드
    -- A < AA < AAA < AAAA < B < C < D
    ...
    SELECT EMPNO, ENAME, MGR
        FROM EMP
        WHERE ENAME = 'FORD';
    -- SQL문은 대소문자 구분 없음, 데이터 대소문자 구별 함
    select empno, ename, mgr
        from emp
        where ename = 'FORD';
...
-- 4. 특정 행 출력 : WHERE절(조건절) - 논리연산자 : AND, OR, NOT
    ...
    -- ex2) 82년도에 입사(HIREDATE)한 사원의 모든 필드
    SELECT * FROM EMP WHERE '81/12/31' < HIREDATE AND HIREDATE < '83/01/01';
    ...
    -- ex3) 연봉이 2400 이상인 직원의 ENAME, SAL, 연봉(SAL*2)을 출력
    ...
    SELECT ENAME, SAL, SAL*12 ANNUALSAL -- 수행순서 (3)
        FROM EMP                        -- 수행순서 (1)
        WHERE SAL*12 > 3000             -- 수행순서 (2)
        ORDER BY ANNUALSAL;             -- 수행순서 (4) => ORDER BY절에는 필드의 별칭 사용 가능
    -- ex5) JOB이 MANAGER이거나 10번 부서(DEPNO)외의 직원의 모든 필드
    SELECT * FROM EMP WHERE JOB='MANAGER' OR DEPTNO != 10;
    
-- 5. 산술연산자(SELECT절, 조건절, ORDER BY절)
    -- 모든 사원의 10% 인상된 월급과 사번을(EMPNO), 이름(ENAME)을 출력
    SELECT EMPNO, ENAME, SAL*1.1 FROM EMP;
    -- 모든 사원의 이름(ENAME), 월급(SAL), 상여(COMM), 연봉(SAL*12+COMM)을 출력
    SELECT ENAME, SAL, COMM, (SAL*12+COMM) AN_INCOME FROM EMP;
    -- 산술연산의 결과는 NULL을 포함하면 결과도 NULL
    -- NVL(NULL일 수도 있는 필드명, NULL을 대체할 값) - 두 매개변수의 타입이 일치
    SELECT ENAME, SAL, NVL(COMM,0), (SAL*12+NVL(COMM,0)) AN_INCOME FROM EMP;
    -- 모든 사원의 사번, 이름, 상사사번 출력. 단, 상사의 사번(MGR)이 없는 직원의 MGR을 'CEO'로 출력
...
-- 8. SQL연산자 (BETWEEN, IN, LIKE, IS NULL)
    -- (1) BETWEEN A AND B : A부터 B까지 (A,B 포함)
        ...
        -- ex3) 이름이 'A', 'B', 'B'로 시작하는 직원의 모든 필드
        SELECT * FROM EMP
    ...
    -- (4) 필드명 IS NULL (해당 필드가 NULL인지 여부)
...
SELECT * FROM EMP ORDER BY SAL; -- SAL기준 오름차순 정렬
SELECT * FROM EMP ORDER BY SAL, ENAME; -- SAL기준 오름차순 정렬(SAL이 같으면 ENAME 오름차순 정렬)
SELECT * FROM EMP ORDER BY SAL DESC; -- SAL기준 내림차순 정렬
SELECT * FROM EMP ORDER BY SAL DESC, HIREDATE DESC; -- SAL기준 내림차순 정렬, 나중 입사자 먼저
...
SELECT * FROM EMP WHERE TO_CHAR(HIREDATE, 'RR/MM/DD') BETWEEN '81/12/31' AND '83/01/01';
...
--      업무, 급여를 검색하시오.
SELECT EMPNO, ENAME, HIREDATE, JOB, SAL FROM EMP
    WHERE TO_CHAR(HIREDATE, 'RR') LIKE '81' AND JOB != 'SALESMAN';
--14.	사번, 사원명, 입사일, 업무, 급여를 급여가 높은 순으로 정렬하고, 
--      급여가 같으면 입사일이 빠른 사원으로 정렬하시오.
SELECT EMPNO, ENAME, HIREDATE, JOB, SAL FROM EMP
    ORDER BY SAL, TO_CHAR(HIREDATE, 'RR/MM/DD');
