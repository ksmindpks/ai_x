...
SELECT EMPNO NO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, D.DEPTNO, DNAME, LOC
    ...
    -- EX2. 급여가(SAL)가 2000이상인 직원의 이름, JOB, 급여, 부서명, 근무지
    ...
    SELECT ENAME, SAL, LOC
        ...
        ORDER BY SAL;
    -- EX5. JOB이 'SALESMAN'이거나 'MANGER'인 사원의 이름, 급여, 상여, 연봉(SAL*12+COMM), 부서명, 근무지(연봉이 큰 순) 출력
    SELECT ENAME, SAL, COMM, SAL*12+NVL(COMM,0) ANUAL_SAL, DNAME, LOC
        FROM EMP E, DEPT D
        WHERE E.DEPTNO=D.DEPTNO AND JOB IN ('SALESMAN', 'MANGER')
        ORDER BY ANUAL_SAL; 
    -- EX6. COMM이 NULL이고, SAL이 2000대인 사원의 이름, 급여, 입사일, 부서번호, 부서명(부서명, 급여 순) 출력
    SELECT ENAME, SAL, HIREDATE, E.DEPTNO, DNAME
        FROM EMP E, DEPT D
        WHERE E.DEPTNO=D.DEPTNO AND COMM IS NULL AND SAL BETWEEN 2000 AND 3000
        ORDER BY DNAME, SAL; 
    ...
    SELECT ENAME, SAL, D.DEPTNO, LOC, COMM 
        FROM EMP E, DEPT D 
...
SELECT * FROM EMP E, SALGRADE S
    ...
    -- EX. 모든 사원의 사번, 이름, JOB, 상사번호, 급여, 급여등급(1등급, 2등급, ...)
    SELECT ENAME, JOB, MGR, SAL, GRADE||'등급' GRADE
        FROM EMP, SALGRADE
        WHERE SAL BETWEEN LOSAL AND HISAL;
    -- EX. 모든 사원의 사번, 이름, JOB, 상사번호, 급여, 급여등급(1등급, 2등급, ...), 부서명
    SELECT ENAME, JOB, MGR, SAL, GRADE||'등급' GRADE, DNAME
    ...
    SELECT ENAME, JOB, GRADE, E.DEPTNO, DNAME, LOC
        FROM EMP E, SALGRADE, DEPT D
        WHERE E.DEPTNO=D.DEPTNO AND COMM IS NOT NULL AND SAL BETWEEN LOSAL AND HISAL;
    -- 탄탄2. 이름, 급여, 입사일, 급여등급
    SELECT ENAME, JOB, HIREDATE, GRADE
        FROM EMP E, SALGRADE
        WHERE SAL BETWEEN LOSAL AND HISAL;
    -- 탄탄3. 이름, 급여, 급여등급, 연봉, 부서명을 부서명순으로 정렬하여 출력. 부서가 같으면 연봉순. 연봉=(sal+comm)*12 comm이 null이면 0
    SELECT ENAME, JOB, GRADE, SAL*12+NVL(COMM,0) ANUAL_SAL, DNAME
        FROM EMP E, SALGRADE, DEPT D
        WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
        ORDER BY DNAME, ANUAL_SAL;
...
SELECT WORKER.EMPNO, WORKER.ENAME, WORKER.MGR, manager.empno B_EMPNO, MANAGER.ENAME B_ENAME
    FROM EMP WORKER, EMP MANAGER
    WHERE WORKER.ENAME='SMITH' AND WORKER.MGR=MANAGER.EMPNO;
    -- EX. 모든 사원의 사번, 이름, 상사의 사번, 상사이름
    SELECT W.EMPNO, W.ENAME, W.MGR, M.ENAME B_ENAME
        FROM EMP W, EMP M
        WHERE W.MGR=M.EMPNO;
    -- EX. 'SMITH'의 상사는 'JONES'이다 포맷으로 출력
    ...
    SELECT M.ENAME, M.DEPTNO
        FROM EMP W, EMP M
        WHERE W.DEPTNO=M.DEPTNO AND W.ENAME='SCOTT'
            AND M.ENAME != 'SCOTT';
    ...
    SELECT C.ENAME, D.LOC
        FROM EMP E, DEPT D, EMP C
        WHERE E.DEPTNO=D.DEPTNO AND E.ENAME='SCOTT' AND C.DEPTNO=E.DEPTNO
            AND C.ENAME != 'SCOTT';
    -- SELECT LOC FROM DEPT D, EMP E WHERE E.DEPTNO=D.DEPTNO AND ENAME='SCOTT';
    SELECT C.ENAME, D.LOC
        FROM EMP C, DEPT D
        WHERE C.DEPTNO=D.DEPTNO AND D.LOC = (SELECT LOC FROM DEPT D, EMP E WHERE E.DEPTNO=D.DEPTNO AND ENAME='SCOTT')
            AND C.ENAME != 'SCOTT';

-- ★ 3. OUTER JOIN ★ -- SELF JOIN, EQUI JOIN시 조건이 만족하지 않는 행까지 나타나게 하는 JOIN
...
SELECT ENAME, JOB, SAL, E.DEPTNO, LOC FROM EMP E, DEPT D 
    WHERE E.DEPTNO=D.DEPTNO AND LOC='NEW YORK';
...
-- Part2
--1. 이름, 직속상사명
SELECT W.ENAME, B.ENAME BOSS
    FROM EMP W, EMP B
    WHERE W.MGR = B.EMPNO;
--2. 이름, 급여, 업무, 직속상사명
SELECT W.ENAME, W.SAL, W.JOB, B.ENAME BOSS
    FROM EMP W, EMP B
    WHERE W.MGR = B.EMPNO;
...
SELECT W.ENAME, W.SAL, W.JOB, B.ENAME BOSS
    ...
    WHERE W.MGR = B.EMPNO(+);
--4. 이름, 급여, 부서명, 직속상사명
SELECT W.ENAME, W.SAL, D.DNAME, B.ENAME BOSS
    FROM EMP W, EMP B, DEPT D
    WHERE W.DEPTNO=D.DEPTNO AND W.MGR = B.EMPNO;
--5. 상사가 없는 직원과 상사가 있는 직원 모두에 대해 이름, 급여, 부서코드, 부서명, 근무지, 직속상사명을 출력하시오(단, 직속상사가 없을 경우 직속상사명에는 ‘없음’으로 대신 출력하시오)
SELECT W.ENAME, W.SAL, D.DEPTNO, D.DNAME, D.LOC, NVL(B.ENAME, '없음') BOSS 
    FROM EMP W, EMP B, DEPT D
    WHERE W.DEPTNO=D.DEPTNO AND W.MGR = B.EMPNO(+);
--6. 이름, 급여, 등급, 부서명, 직속상사명. 급여가 2000이상인 사람
SELECT W.ENAME, W.SAL, S.GRADE, D.DNAME, B.ENAME BOSS 
    FROM EMP W, EMP B, DEPT D, SALGRADE S
    WHERE W.DEPTNO=D.DEPTNO AND W.MGR = B.EMPNO
        AND W.SAL >= 2000 AND W.SAL BETWEEN LOSAL AND HISAL;
...
SELECT W.ENAME, W.SAL, S.GRADE, D.DNAME, (W.SAL+NVL(W.COMM,0))*12 ANUAL_SAL, B.ENAME BOSS 
    FROM EMP W, EMP B, DEPT D, SALGRADE S
    WHERE W.DEPTNO=D.DEPTNO AND W.MGR = B.EMPNO
        AND W.SAL BETWEEN LOSAL AND HISAL;
...
SELECT W.ENAME, W.SAL, S.GRADE, D.DNAME, (W.SAL+NVL(W.COMM,0))*12 ANUAL_SAL, B.ENAME BOSS 
    ...
    WHERE W.DEPTNO=D.DEPTNO AND W.MGR = B.EMPNO
        AND W.SAL BETWEEN LOSAL AND HISAL
        ORDER BY D.DNAME, W.SAL;
--10. 사원테이블에서 사원명, 사원의 상사를 검색하시오(상사가 없는 직원까지 전체).
SELECT W.ENAME, B.ENAME BOSS 
    FROM EMP W, EMP B
    WHERE W.MGR = B.EMPNO(+);
--11. 사원명, 상사명, 상사의 상사명을 검색하시오(self join)
SELECT W.ENAME, B.ENAME BOSS, SB.ENAME SUPERBOSS 
    FROM EMP W, EMP B, EMP SB
    WHERE W.MGR = B.EMPNO AND B.MGR = SB.EMPNO;
--12. 위의 결과에서 상위 상사가 없는 모든 직원의 이름도 출력되도록 수정하시오(outer join)
SELECT W.ENAME WORKER, B.ENAME BOSS, SB.ENAME SUPERBOSS 
    FROM EMP W, EMP B, EMP SB
    WHERE W.MGR = B.EMPNO(+) AND B.MGR = SB.EMPNO(+);